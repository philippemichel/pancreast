...
ttb <- tt |> 
  dplyr::filter(suivi <61) |> 
  dplyr::filter(date.de.diagnostic > "2010/01/01") 
...
ggsurvplot(
  ...
  surv.median.line = "hv",
  conf.int = TRUE,
  legend.labs = c("Chirurgical","Non chirurgical"),
  legend = "top",
...
ttb <- ttb |> 
  filter(traitement != "Palliatif")
```


`r nrow(ttb)` patients n'ont pas été opérés soit `r round(100*nrow(tta)/nrow(tt),1)`%  de la population. Parmi ceux-ci, `r zzn` étaient considérés d’emblée comme en traitement palliatif. On exclu ces patients palliatifs.
...
Les items T & N de la classification TNM n'a pas été retenus dans le modèle final. L'âge ne semble pas être lié au pronostic.
...
ttb <- ttb |> 
  mutate(meta = if_else(metastase == "non", "Non", "Oui" ))
ff <- Surv(ttb$suivi, ttb$dcd)
coxph(ff ~ tranche.age + meta + grade + ganglions.envahis,  data = ttb) |> 
  tbl_regression(exponentiate = TRUE) |> 
  modify_header(label ~ " ") %>%
  bold_labels() |> 
  bold_p() |> 
